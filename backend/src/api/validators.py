...
def validate_move(game: Game | None, move: MoveInput | None) -> None:
    ...
    if move.player != game.next_player_to_move_username:  # type: ignore [union-attr]
        raise WrongPlayerToMoveError()

    row = calculate_row_by_col(game.board, move.col)  # type: ignore [union-attr]
    if not is_valid_move(game.board, row, move.col):  # type: ignore [union-attr]
        raise MoveNotValidError()
